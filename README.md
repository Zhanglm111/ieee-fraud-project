...
随着互联网支付、电商交易和数字金融业务的发展，互联网交易欺诈呈现出高频、隐蔽、跨设备和批量化等特征。传统基于固定规则的风控方法虽然具有较强可解释性，但在面对复杂关联行为和快速变化的欺诈模式时，容易出现规则滞后、召回不足和维护成本较高的问题。因此，如何基于交易、身份、设备和行为等多源信号构建可复现、可解释且具备排序能力的欺诈风险模型，是交易风控场景中的核心问题。

互联网交易欺诈具有高维、多源、强不平衡等特点，需要结合风险特征工程和可解释机器学习方法识别潜在异常交易。本项目基于公开匿名交易欺诈数据，模拟互联网交易风控场景，构建一套模块化、可解释以及可复现的离线交易欺诈识别与风险评分流程。项目重点不是单一 Notebook 实验，而是将数据处理、风险特征体系、WOE-IV 特征筛选、模型训练、评估与 SHAP 解释整合成一条完整的风控建模流程。

项目使用公开匿名交易欺诈数据进行实验验证，用于模拟互联网交易风控场景。当前实验采用 IEEE-CIS Fraud Detection 数据，不代表特定企业或平台的真实业务数据。
...
互联网交易欺诈通常不是单一字段异常，而是多个字段组共同作用。本项目在代码中使用 `FeatureCategory` 和 `RiskMechanism` 两层标签描述风险信号，并在训练输出中保存对应的特征体系表。

### 字段分类与风险机制口径

本项目在代码中使用两层标签描述风险信号：`FeatureCategory` 表示字段在数据结构上的类别，`RiskMechanism` 表示该类字段在风控解释中的风险归因。
...
当前结果显示，XGBoost 在各特征方案上的整体表现明显优于 WOE-LR 和 Random Forest。使用`iv_002_050 + XGBoost` 作为主解释方案，原因不是它在所有指标上都最高，而是它在召回能力、风险机制解释和去除极端 IV 变量之间取得了较好的平衡。
...
| `iv_002_050` | 0.8983 | 0.5064 | 0.5074 | 0.4794 | 0.4930 | 0.6441 | 0.7950 | Recall 最高，更适合召回优先的风控场景 |
...
当前项目已经形成了从数据处理、特征筛选、模型训练到解释分析的完整离线流程，但仍有进一步完善空间：

- **概率解释仍需校准**：当前 XGBoost 输出主要作为风险排序分使用，不应直接解释为真实欺诈概率。后续可引入 Platt Scaling 或 Isotonic Regression，并使用 Brier Score、LogLoss 和校准曲线评估概率可靠性。
- **业务策略需要回测**：当前结果主要基于离线 Train、Valid、Test 切分评估。若进入业务应用阶段，需要按时间顺序回放历史交易，结合误杀成本、审核成本、欺诈损失和审核容量评估策略收益。
- **WOE-LR 仍有轻微改进空间**：TimeSeriesSplit CV WOE 已降低训练集内部目标泄露，但最早一段训练样本缺少历史 holdout，当前使用 full encoder 回填。更严格的方式可以使用全局先验 WOE 回填，或丢弃最早一小段训练样本。
- **特征语义仍受匿名字段限制**：`V*`、部分 `id_*` 字段具有较强预测价值，但业务含义不完全透明。后续如果有更完整的字段字典或真实业务日志，可以进一步提升风险归因的可解释性。
- **模型稳定性还可加强**：后续可增加跨时间窗口验证、分群稳定性分析、PSI/CSI 漂移监控和阈值敏感性分析，使模型更接近真实风控上线要求。
