...
随着线上支付、电商交易和数字金融业务的发展，交易欺诈呈现出高频、隐蔽、跨设备和批量化等特征。传统基于固定规则的风控方法虽然具有较强可解释性，但在面对复杂关联行为和快速变化的欺诈模式时，容易出现规则滞后、召回不足和维护成本较高的问题。因此，如何基于交易、身份、设备和行为等多源信号构建可复现、可解释且具备排序能力的欺诈风险模型，是在线风控场景中的核心问题。

在线交易欺诈具有高维、多源、强不平衡等特点，需要结合风险特征工程和可解释机器学习方法识别潜在异常交易。本项目面向 **Online Transaction Fraud Risk Modeling** 场景，构建一套模块化、可解释以及可复现的交易欺诈识别与风险评分流程。项目重点不是单一 Notebook 实验，而是将数据处理、风险特征体系、WOE-IV 特征筛选、模型训练、评估与 SHAP 解释整合成一条完整的风控建模流程。
...
在线交易欺诈通常不是单一字段异常，而是多个字段组共同作用。本项目在代码中使用 `FeatureCategory` 和 `RiskMechanism` 两层标签描述风险信号，并在训练输出中保存对应的特征体系表。
...
- **线上策略需要回测**：当前结果主要基于离线 Train、Valid、Test 切分评估。若进入业务应用阶段，需要按时间顺序回放历史交易，结合误杀成本、审核成本、欺诈损失和审核容量评估策略收益。
